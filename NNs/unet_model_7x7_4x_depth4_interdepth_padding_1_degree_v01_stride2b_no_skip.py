...
# CONVOLUTION LAYERS
# --> Add 7x7 convolution for first layer
# --> padding = "same"
...
# --> nn.ELU or nn.GLUMagistrska_naloga/U-Net/Experiment_33_1hr/Code_0_trenutno_1hr/unet_model_7x7_4x_depth3_interdepth_padding_1_degree_v01_stride2b_no_skip.py
...
def conv7x7_stride2(in_channels, out_channels, stride=2, 
            padding=0, bias=True, groups=1):    
    """3x3 convolution layer"""
    return nn.Conv2d(
        in_channels,
...
def upconv4x4(in_channels, out_channels, mode='transpose', output_padding=(0,0)):
    """2x2 upsampling"""
    # Convolutional upsampling
    if mode == 'transpose':
        return nn.ConvTranspose2d(
        ...
        # Linear interpolation in upsampling
        return nn.Sequential(
            nn.Upsample(mode='bilinear', scale_factor=4),
            conv1x1(in_channels, out_channels))
...
class DownConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, depth, pooling=True):
        ...
        if depth == 0:
            ...
            self.conv2 = conv7x7_stride2(self.in_channels + self.out_channels, self.out_channels)
    ...
    def forward(self, x, depth, p):
        ...
        # print(f"before_pool={before_pool.shape}")
        # before_pool = x
        if self.pooling:
            x = self.pool(x)
...
class UpConv(nn.Module):
    ...
    A ReLU activation follows each convolution.
    """
    def __init__(self, in_channels, out_channels, depth, total_depth,
        ...
        if depth == total_depth - 2:
            # print(f"blabla depth / total_depth = {depth} / {total_depth}")
            self.upconv_final_x = upconv2x2(
                self.in_channels,
                self.out_channels, 
                mode=self.up_mode,
                output_padding=OUTPUT_PADDING[depth])
            self.upconv_final_x0 = upconv2x2(
                self.in_channels,
                self.out_channels, 
                mode=self.up_mode,
                output_padding=OUTPUT_PADDING[depth])
    ...
    def forward(self, from_down, from_up, depth, total_depth, p):
        """ Forward pass
        Arguments:
        ...
        if depth == total_depth - 2:
            x0 = self.upconv_final_x0(x0)
            x = self.upconv_final_x(x)
...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, depth=5, 
        ...
        for i in range(depth-1):  # to je za resolucijo 60 x 120
        # for i in range(depth): # to je za resolucijo 180 x 360 in stride=2 v prvem koraku
            ins = outs
            outs = ins # // 2

            # Get x from decoding block
            # print("depth=",depth)
            up_conv = UpConv(ins, outs, i, depth, up_mode=up_mode,
    ...
    def forward(self, x, p=0.):
        ...
        for i, module in enumerate(self.up_convs):
            ...
            before_pool = None
            x = module(before_pool, x, i, depth, p)
            # print(i, x.shape)
...
if __name__ == "__main__":
    ...
    model = UNet(in_channels=1*24+3, out_channels=24, depth=4, start_filts=100,
        up_mode='transpose', merge_mode='concat')
    x = Variable(torch.FloatTensor(np.random.random((1, 27, 180, 360))))
    out, skip_connections = model(x)
    loss = torch.sum(out)
    loss.backward()
