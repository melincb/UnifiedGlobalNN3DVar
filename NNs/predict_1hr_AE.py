...
class Predicting():
        ...
        if True:
            ...
            if action == "decode":
                ...
                for i in range(N):
                    ...
                    # Ker je pri učenju uporabljen DataParalell, moramo funkcijo encode klicati posredno prek MODELS[i].module
                    # DIRECT_PRED = MODELS[i].module.decode(z, shape, p=0) # p je verjetnost za dropout - to je 0.
                    DIRECT_PRED = MODELS[i].module.decode(z, p=0) # p je verjetnost za dropout - to je 0.

                # print(DIRECT_PRED.shape)
                ...
                for i in range(len(SCALER_NAMES)):
                    # Inverzno skaliranje
                    # for i in range(len(SCALER_NAMES)):
                    pred = SCALERS[i].inverse_transform(DIRECT_PRED[:,i])
                    # pred = pred[0,0,:,:]
                    # pred = pred[0,:,:,:]
                    PRED[:,i] = pred

                # return DIRECT_PRED.cpu().detach().numpy()
                ...
                if action == "encode-decode":
                    for i in range(N):
                        if type(STATIC_TENSOR) == type(1):
                            ...
                            DIRECT_PRED, Z = MODELS[i](torch.cat((SCALED_TENSOR, STATIC_TENSOR), dim=1))
                    ...
                    return PRED.cpu().detach().numpy(), Z.cpu().detach().numpy()
                    ...
                    for i in range(N):
                        if type(STATIC_TENSOR) == type(1):
                            # To se izvede, če nimamo statičnih polj v modelu
                            # Ker je pri učenju uporabljen DataParalell, moramo funkcijo encode klicati posredno prek MODELS[i].module
                            Z = MODELS[i].module.encode(SCALED_TENSOR, p=0) # p je verjetnost za dropout - to je 0.
                            # Z = MODELS[i].module.reparameterize(MU, LOGVAR)
                        else:
                            # To se izvede, če imamo statična polja
                            # Ker je pri učenju uporabljen DataParalell, moramo funkcijo encode klicati posredno prek MODELS[i].module
                            Z = MODELS[i].module.encode(torch.cat((SCALED_TENSOR, STATIC_TENSOR), dim=1), p=0) # p je verjetnost za dropout - to je 0.
                            # Z = MODELS[i].module.reparameterize(MU, LOGVAR)
    ...
    def encode_decode(self, root_annotations, ANNOTATIONS_NAMES,
        ...
        ---> action: One of {"encode-decode", "encode", "decode"} <- what action do you want to do?
        ---> delta_t = 1./24 <- delta t in fraction of the day
        ---> z: latent vector to be decoded if action == "decode"
        ---> use_ERA5 (bool): if False, insert your own list_of_input_fields (shape (24,1,180,360)) - NOT STANDARDIZED!
        ---> list_of_input_fields (24,1,180,360) - NOT STANDARDIZED
        Returns:
        ---> fields, preds, z, mu, logvar (depending od action) - lists of truths and predictions, first element
        ...
        while num_pred < num_of_prediction_steps:
            ...
            if action == "encode-decode":
                predi, z = self.predict_next(
                    MODELS=MODELS,
                    INPUT_SEQUENCE=np.array([list(itertools.chain(*INPUT_SEQUENCE[:]))])[:,:input_sequence_len*len(ANNOTATIONS_NAMES)],
    ...
    def decode(self,ANNOTATIONS_NAMES, MODELS,
                    root_scaler, SCALER_NAMES, SCALER_TYPES, 
                    num_of_prediction_steps, z):
        """
        Args:
        ---> MODELS: list of pytorch model architectures
        ---> num_of_prediction_steps: how many steps do you wish to predict
        ...
        PREDS = []
        for i in range(len(ANNOTATIONS_NAMES)):
            # preds = [np.random.rand(180, 360)] # [FIELDS[i][0]]
            preds = [np.zeros((180, 360), dtype=float)] # [FIELDS[i][0]]
            PREDS.append(preds)
