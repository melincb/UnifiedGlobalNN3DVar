...
class DataWriter():
    ...
    def save_individual_files(self, year, fieldName, fileName,
        ...
        daily_timesteps = True --> currently only implemented for either daily timestapes of timestep_hr hr timestapes
        timestep_hr = numer of hours in timestep
        """

        assert timestep_hr in [1,2,3,4,6,12,24], "Nesprejemljiv timestep_hr - ni v množici {1,2,3,4,6,12,24}!"
    ...
    def save_individual_files_static_fields(
        ...
        # Define current coordinates
        ln_c = np.arange(-180,180,1.0)
        lt_c = np.arange(-90,90.1,1.0)
        ...
        if save:
            ...
            if flip_and_roll:
                ...
                # print(lt_c, ln_c, field[0])
                # print(lt_c.shape, ln_c.shape, field[0].shape)
    ...
    def save_individual_files_nan_values(self, year, fieldName, fileName,
        ...
        daily_timesteps = True --> currently only implemented for either daily timestapes of timestep_hr hr timestapes
        timestep_hr = numer of hours in timestep
        replace_nan_with = {stevilo ali "povprecje"}
        # POPRAVLJENO ZA DT=1HR
        """
        ...
        print(f"replace_nan_with={replace_nan_with}")

        if replace_nan_with == "povprecje": # Nadomesti nan s povprecjem
            povprecje = np.nanmean(np.array(field))
            print(f"povprecje = {povprecje}")
            field = np.nan_to_num(np.array(field), copy=False, nan=povprecje)
        else: # Nadomesti nan s številko
            print("b")
    ...
    def save_annotations_file(self, year, fieldName, fileName,
        ...
        if contains_nan == False:
            list_of_paths = self.save_individual_files(
                year, fieldName, fileName, save=False, daily_timesteps = daily_timesteps, timestep_hr = timestep_hr
                )
        else:
...
class DateToConsecutiveDay():
        ...
        def convert_date_to_consecutive_day(date, delta_t = 1):
            ...
            --> delta_t [day] (float): if you don't have days but e.g.
                                 dt = 12 h => delta_t = 0.5
            Returns:
        ...
        def num_of_samples(year=2021, month=12, day=31, delta_t=1,
            ...
            with start date 1970 so 01.01.1970 at 0 hr is index 0.
            Indexes are correct (end index is included).
            delta_t in days (12 hr --> delta_t = 0.5)
            """
            from datetime import date
        ...
        @staticmethod
        def index_to_date(index, delta_t=1, start_year=1970, start_month=1, start_day=1):
            """
            ...
            day_index = (timedelta(int(index*delta_t)) + day_beginning)
...
class CalculateClimatology():
    ...
    --> clima_time_series_from_date_hr
    --> clima_time_series_from_consecutive_day_hr
    """
    ...
    def calculate_and_save(self, start_year, end_year):
        ...
        for consecutive_day in range(1, 366):
            ...
            for hr in range(0,24,1): # [0, 12]:
                # if hr == 0: pristej = 0
                # elif hr == 12: pristej = 1

                year_averages = []
                for year in range(start_year, end_year+1):
                    
                    # I don't subtract index, because no data is missing.
                    index0 = date_to_day.num_of_samples(year, month, day, delta_t) + hr # pristej
                    ...
                    for t in range(1, 6):
                        # Days before
                        index_minus_t = index0 - t*int(1/delta_t)
                        path_minus_t = annotation_file_geopotencital[index_minus_t]
                        field_minus_t = np.load(path_minus_t)
                        Fields.append(field_minus_t)
                        # Days after
                        index_plus_t = index0 + t*int(1/delta_t)
                        path_plus_t = annotation_file_geopotencital[index_plus_t]
                ...
                np.save(
                    os.path.join(
                    self.root_save_load, "climatology" + "_" + str(consecutive_day) + "_" + str(hr)),
                    consecutive_day_climatology
                    )

    def load_day_climatology_from_consecutive_day_hr(
        self, consecutive_day, hr):
        """
        Args:
        ...
        # consecutive_day = 2 * consecutive_day - odstej
        # consecutive_day = consecutive_day - odstej

        path = os.path.join(
            self.root_save_load, "climatology" + "_"
            + str(consecutive_day) + "_"
            + str(hr) + ".npy")
    ...
    def load_day_climatology_from_date_hr(self, date, hr):
        """
        Args:
        ---> date (tuple): (day, month)
        ---> hr \in {0,1,2,3,...,23}
    ...
    def clima_time_series_from_date_hr(self, start_date,
        ...
        --> hr \in {0} # start of sequence hour (initial time)
        --> delta_t = {1./24, 1./12, 1./8, 1./6, 1./4, 1./3, 1./2, 1.} <- delta t in fraction of the day
        Returns:
        --> climas = [clima1, clima2, ..., clima_num_of_pred_steps]
        """
        # assert hr == 0, "hr != 0, ni ok za data_preparation_1hr.clima_time_series_from_date_hr"
        assert delta_t in {1./24, 1./12, 1./8, 1./6, 1./4, 1./3, 1./2, 1.}, "delta_t ni ok za data_preparation_1hr.clima_time_series_from_date_hr"
        ...
        consecutive_day = date_to_day.convert_date_to_consecutive_day(
            start_date, delta_t=1.) # Tukaj je delta_t=1 ker želimo samo indeks za ta datum?
        consecutive_hr = hr
        ...
        # When to change date:
        if int(1/delta_t) == 24:
            consecutive_hr_limit = 23
            delta_hr = 1
        elif int(1/delta_t) == 12:
            consecutive_hr_limit = 22
            delta_hr = 2
        elif int(1/delta_t) == 8:
            consecutive_hr_limit = 21
            delta_hr = 3
        elif int(1/delta_t) == 6:
            consecutive_hr_limit = 20
            delta_hr = 4
        elif int(1/delta_t) == 4:
            consecutive_hr_limit = 18
            delta_hr = 6
        elif int(1/delta_t) == 3:
            consecutive_hr_limit = 16
            delta_hr = 8
        elif int(1/delta_t) == 2:
            consecutive_hr_limit = 12
            delta_hr = 12
        elif int(1/delta_t) == 1:
            consecutive_hr_limit = 0
            delta_hr = 0
        ...
        for i in range(num_of_prediction_steps+1):
            ...
            for j in range(averaging_sequence_len):
                day_clima = self.load_day_climatology_from_consecutive_day_hr(
                    consecutive_day=consecutive_day, hr=consecutive_hr)
                intermediate_day_clima.append(day_clima)
                if consecutive_hr == consecutive_hr_limit:
                    consecutive_day += 1
                    consecutive_hr = 0
                else:
                    consecutive_hr += delta_hr
                # consecutive_day += 1
                # Će pridemo čez 1. januar
                if consecutive_day > 365:
                    consecutive_day = 1
    ...
    def clima_time_series_from_consecutive_day_hr(
...
if __name__ == "__main__":
    clima = CalculateClimatology(
        "/home/perkanu/Magistrska_naloga/U-Net/Data_1hr/Original_Density",
        "geopotential_annotation_file.txt",
        "/home/perkanu/Magistrska_naloga/U-Net/Data_1hr/Original_Density/Geopotential_500")

    # clima.calculate_and_save(1951, 2020)

    day1 = clima.load_day_climatology_from_consecutive_day_hr(255,0)
    ...
    day2 = clima.load_day_climatology_from_date_hr((25, 10), 0)
    ...
    print("a")
    climas = clima.clima_time_series_from_date_hr(start_date=(25, 12),
                num_of_prediction_steps=20, averaging_sequence_len=1, hr=12)

    print("b")
    climas = clima.clima_time_series_from_consecutive_day_hr(
            start_day=359, num_of_prediction_steps=20,
            averaging_sequence_len=1, hr=12)
